...
import { AlertTriangle, Save, Palette, Bell, Store, Upload, Users, UserPlus, Trash2, Edit2, X } from 'lucide-react';
...
const Settings: React.FC = () => {
  ...
  const [userPassword, setUserPassword] = useState(''); // Handle password separately
  ...
  const handleLogoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      try {
        ...
        alert('Error uploading file. It might be too large.');
      }
    }
  };
  ...
  const handleSaveUser = (e: React.FormEvent) => {
    ...
    // If editing, password is optional (only if changing)
  ...
  const openUserModal = (user?: User) => {
    if (user) {
      setCurrentUser(user);
      setUserPassword(''); // Don't show existing hash
    } else {
