...
import { AlertTriangle, Save, Palette, Bell, Store, Upload, Users, UserPlus, Trash2, Edit2, X, Shield, Database, Download, RefreshCw, Stethoscope } from 'lucide-react';
...
const Settings: React.FC = () => {
  ...
  useEffect(() => {
    // Always fetch doctors when mounting settings to ensure dropdowns are populated
    setUsers(getUsers());
  ...
  const handleSaveUser = (e: React.FormEvent) => {
    ...
    // Ensure relatedId is cleared if role is not Doctor
    const relatedId = currentUser.role === Role.Doctor ? currentUser.relatedId : undefined;
  ...
  return (
    <>
      <div className="max-w-5xl mx-auto space-y-8 pb-10 animate-fade-in-up">
            ...
            {activeTab === 'notifications' && (
              <div className="space-y-6">
                <div className="bg-blue-50 p-6 rounded-2xl border border-blue-100 mb-6">
                  <h3 className="font-bold text-blue-800 mb-2 flex items-center"><Bell size={18} className="mr-2"/> Automated Messaging</h3>
                  <p className="text-sm text-blue-600">Configure the templates used for automatic email reminders and follow-up sequences.</p>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="space-y-4">
                     <label className="block text-sm font-black text-gray-700 uppercase tracking-widest">Appointment Reminder</label>
                     <textarea 
                        className={inputClass + " font-mono text-sm h-48"} 
                     ...
                     />
                     <p className="text-[10px] text-gray-400 font-bold uppercase">Variables: {'{patient_name}, {date}, {time}, {doctor_name}'}</p>
                  </div>
                  <div className="space-y-4">
                     <label className="block text-sm font-black text-gray-700 uppercase tracking-widest">Post-Visit Follow-up</label>
                     <textarea 
                        className={inputClass + " font-mono text-sm h-48"} 
                     ...
                     />
                     <p className="text-[10px] text-gray-400 font-bold uppercase">Variables: {'{patient_name}, {doctor_name}'}</p>
                  </div>
                </div>
                <div className="pt-4">
                  <button onClick={handleSaveSettings} className="bg-primary text-white px-8 py-3 rounded-2xl font-black hover:bg-secondary transition flex items-center">
                         ...
                         <tr key={u.id} className="hover:bg-gray-50 transition">
                           ...
                           <td className="p-5">
                             <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest border ${
                               ...
                               {u.role}
                               {u.role === Role.Doctor && u.relatedId && (
                                 <span className="ml-2 text-gray-400 font-normal normal-case">
                                   → {doctors.find(d => d.id === u.relatedId)?.name || 'Unknown Link'}
                                 </span>
                               )}
                             </span>
                           </td>
          ...
          <div className="bg-white rounded-[40px] shadow-2xl w-full max-w-md overflow-hidden animate-fade-in-up">
             <div className="p-10">
                ...
                <form onSubmit={handleSaveUser} className="space-y-6">
                   ...
                   {/* DYNAMIC DOCTOR LINKING FIELD */}
                   {currentUser.role === Role.Doctor && (
                     <div className="animate-fade-in-up bg-blue-50 p-4 rounded-xl border border-blue-100">
                        <label className="block text-[10px] font-black text-blue-500 uppercase tracking-widest mb-2 flex items-center">
                          <Stethoscope size={12} className="mr-1"/> Link to Physician Profile
                        </label>
                        <select 
                          required 
                          className={inputClass} 
                          value={currentUser.relatedId || ''} 
                          onChange={e => setCurrentUser({...currentUser, relatedId: e.target.value})}
                        >
                           <option value="">-- Select Medical Staff Profile --</option>
                           {doctors.map(d => (
                             <option key={d.id} value={d.id}>{d.name} ({d.specialty})</option>
                           ))}
                        </select>
                        <p className="text-[10px] text-blue-400 mt-2 font-bold">
                          * Required to enable doctor-specific dashboard views.
                        </p>
                     </div>
                   )}
